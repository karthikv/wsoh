...
/****** ATTACHMENT VIEWING PERIODIC FUNCTIONS *********************************/
        ...
        (function processAttachments() {
            var a = 'a[data-expanded-url^="' + baseURL + '"]';
...
/****** ATTACHMENT ADDING FUNCTIONS *******************************************/
                        ...
                        function( status, response ) {
                            switch( status ) {
                                ...
                                case 200:
                                    ...
                                    // newly created. Delete the stored hash to
                                    // force Dropbox to present new data.
                                    LocalStorage.delete( 'Public/pigeon-carrier-hash' );
        ...
        else
        {
            ...
            appendFileToAttachmentList = function( file, select ) {
                ...
                if( inList[ fileName ] )
                {
                    ...
                    if( select )
                        $( 'a[data-name="' + fileName + '"]', $attachmentList )
                            .addClass( 'selected' );
            ...
            $addAttachment.click( function( event ) {
                ...
                if( rCarryURL.test( val ) ) {
                    // $1 is the base URL
                    val = val.replace( rCarryURL, '$1' + encodedData );
                    $tweetTextarea.val( val );

                    var linkIndex = val.indexOf( baseURL + encodedData );
                    $tweetTextarea[0].setSelectionRange( linkIndex, linkIndex
                        + baseURL.length + encodedData.length );
                }
                // otherwise, add one
                else {
    ...
    chrome.extension.onConnect.addListener( function( port ) {
        console.assert( port.name == 'uploadProgress' );
